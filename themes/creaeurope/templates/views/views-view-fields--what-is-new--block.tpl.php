...
/**
 * Views-view-fields--what-is-new--block.tpl.php
 * PHP version 5
 *
 * @category Production
 * @package  Creaeurope
...
<article class="hp-whatsnew-box">
    ...
    <?php switch ($fields['type']->raw) {
    case 'external_calls':{
        print l(
            $fields['type']->content, 
            'calls',
            array('attributes' => array('class' => array('calls-link')))
        );
                break;
    }
    case 'news':{
        print l(
            $fields['type']->content,
            'news',
            array('attributes' => array('class' => array('news-link')))
        );
                break;
    }
    case 'events':{
        print l(
            $fields['type']->content,
            'events',
            array('attributes' => array('class' => array('events-link')))
        );
                break;
    }
    default:{
    ...
    <?php if ($fields['type']->raw == 'external_calls') : ?>
        ...
        <?php print $fields['field_calls_deadline']->content; ?>
			</p>
    <?php endif ?>
	</div>
